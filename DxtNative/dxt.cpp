...
static inline void decode_dxt3_block(const uint64_t *data, uint32_t *outbuf) {
	int a[16];
	uint8_t* dataptr = (uint8_t*)data;
	for (int i = 0; i < 16; i += 2)
	{
		a[i] = ((*dataptr & 0xF) * 0x11) << 24;
		a[i + 1] = ((*dataptr >> 4) * 0x11) << 24;
		dataptr++;
	}

	int r0, g0, b0, r1, g1, b1;
	int q0 = ((uint16_t*)data)[4];
	int q1 = ((uint16_t*)data)[5];
	rgb565(q0, &r0, &g0, &b0);
	rgb565(q1, &r1, &g1, &b1);
	...
	uint_fast32_t d = *(data + 1) >> 32;
	for (int i = 0; i < 16; i++, d >>= 2)
		outbuf[i] = c[d & 3] | a[i];
}

void decode_dxt3(const uint64_t *data, const int w, const int h, uint32_t *image) {
	int bcw = (w + 3) / 4;
	int bch = (h + 3) / 4;
	int clen_last = (w + 3) % 4 + 1;
	uint32_t buf[16];
